...
def visualize_boxes_and_labels_yolo(frame, boxes_, classes, labels_, scores_, class_list, use_normalized_coordinates):
    """Overlay labeled boxes on an image with formatted scores and label names.
     ...
     Args:
       ...
       class_list : class_list(car, person....) some are removed from original COCO dataset
       use_normalized_coordinates: whether boxes is to be interpreted as
         normalized coordinates or not.
     """
    ...
    for i in range(len(boxes_)):
        ...
        # draw boxes and labels if it exists in class_list
        # if not it means that user does not want the object to be detected
        if classes[labels_[i]] in class_list:
